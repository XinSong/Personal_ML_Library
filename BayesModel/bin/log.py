...
# Global logging produced a uniq bd_logger
...
def init(level, normal_log_path, wf_log_path = None, auto_rotate = False, backup_days = 7):
    ...
    bd_fmt = logging.Formatter('%(levelname)s: %(asctime)s: %(thread)d [%(bd_filename)s:%(bd_lineno)d][%(bd_funcName)s] %(message)s')
    # Set normal log file
    if auto_rotate:
        backup_count = backup_days * 24
        normal_hdlr = logging.handlers.TimedRotatingFileHandler(normal_log_path, when = 'h',
    ...
    if wf_log_path != None:
        ...
        # set wf log file
        if auto_rotate:
            wf_hdlr = logging.handlers.TimedRotatingFileHandler(wf_log_path, when='h',
                                                         interval = 1, backupCount = backup_count)
        else:
